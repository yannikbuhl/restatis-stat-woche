...
-   Szenario: Wir sollen eine Analyse zum Thema "Heizungsarten" durchführen, basierend auf der Gebäude- und Wohnungszählung (GWZ) und nutzen hier für die Zensusdatenbank
...
-   Alle Funktionen können für für alle APIs genutzt werden, <br> für Teile von ihnen oder für nur eine
...
-   Daten & Metadaten finden
    ...
    -   `gen_objects2stat()`: Function to find objects related to a statistic

    -   `gen_objects2var()`: Function to find objects related to a variable

    -   `gen_val2var()`: Function to extract the possible values from a variable

    -   `gen_val2var2stat()`: Get values from variables from a statistic

    -   `gen_search_vars()`: Function to search for specific variables

-   Hilfsfunktionen

    -   `gen_modified_data()`: Function to check for updates, changes, or new objects based on a specific date

    -   `gen_alternative_terms()`: Function to find search terms that are similar or related to one another in spelling

    -   `gen_signs()`: Function to list all currently used special signs (e.g., 0, \*, X, (), p, ...) and their meaning

    -   `gen_update_evas()`: Function to scrape EVAS numbers from the EVAS website
